document.addEventListener('DOMContentLoaded', function() {
  ...
  async function loadSavedFilters() {
    ...
    try {
      ...
      const siteFilters = savedFilters[hostname] || [];
      
      console.log(`[Popup] Found ${siteFilters.length} saved filters for ${hostname}`);
      
      if (siteFilters.length === 0) {
        savedFiltersDiv.innerHTML = '<p style="text-align: center; color: #666;">No saved filters for this site</p>';
        return;
      }
      
      // Sort by most recent first
      siteFilters.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
      ...
      siteFilters.forEach(filterSet => {
        ...
        const formatFilterText = (filters) => {
          ...
          return filters.map(f => f.text || f.value || f.id).join(', ');
        };
        ...
        deleteBtn.addEventListener('click', () => deleteFilterSet(hostname, filterSet.id));
  ...
  async function applyFilterSet(filterSet) {
    ...
    try {
      ...
      if (!tab.url.includes('ajio.com') && !tab.url.includes('myntra.com')) {
  ...
  // Delete a filter set
  async function deleteFilterSet(hostname, id) {
    console.log("[Popup] Deleting filter set:", id);
    try {
      const {savedFilters = {}} = await chrome.storage.local.get(['savedFilters']);
      
      if (savedFilters[hostname]) {
        savedFilters[hostname] = savedFilters[hostname].filter(set => set.id !== id);
        await chrome.storage.local.set({savedFilters: savedFilters});
